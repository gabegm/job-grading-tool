...
import type { Company, AnnualRevenue, GlobalHeadcount, GeographicFootprint, CorporateStructure, RoleTrack, FactorWeighting, FactorLevel } from '../../src/lib/types';
...
// ─── Banding Decision Tree Integration Tests ────────────────────────

describe('banding decision tree integration', () => {
  const companyCeiling = { grade: 18, gradeLabel: 'Corporate Officer' };
  const defaultBands = [
  ...
  it('returns recommendedBand for complex + large (band1)', () => {
    const result = scoreRole(
      companyCeiling,
      '', // no explicit band
      {
        jobFunctionalKnowledge: 45,
        businessExpertise: 40,
        leadership: 40,
        problemSolving: 40,
        natureOfImpact: 40,
        areaOfImpact: 40,
        interpersonalSkills: 40,
      },
      { managesTeam: true, decisionAutonomy: true, financialAuthority: 40 },
      'manager',
      undefined,
      false,
      'complex',
    ...
    expect(result.recommendedBand).toBe('band1');
  });

  it('returns recommendedBand for homogeneous + small (band3)', () => {
    const result = scoreRole(
      companyCeiling,
      '', // no explicit band
      {
    ...
    expect(result.recommendedBand).toBe('band3');
  });

  it('uses explicit band over recommended when both provided', () => {
    const result = scoreRole(
      companyCeiling,
      'band2', // explicit band overrides recommendation
      {
        jobFunctionalKnowledge: 45,
        businessExpertise: 40,
        leadership: 40,
    ...
    // recommendedBand is still 'band1' (the recommendation), but scoring uses 'band2' (explicit)
    expect(result.recommendedBand).toBe('band1');
    // Grade should be calculated using band2 (middle management), not band1 (executive)
    // band2 max is 18, so grade should be <= 18 (and < 18 since non-CEO)
    expect(result.grade).toBeLessThanOrEqual(17);
  });

  it('returns undefined recommendedBand when complexity/size not provided', () => {
    const result = scoreRole(
      companyCeiling,
      'band3',
      {
    ...
    expect(result.recommendedBand).toBeUndefined();
  });
});

// ─── Factor Level Validation Tests ──────────────────────────────────

describe('factor level validation', () => {
  const companyCeiling = { grade: 18, gradeLabel: 'Corporate Officer' };
  const defaultBands = [
    { id: 'band1', label: 'C-Suite / Board', range: 'Grades 13–25' },
    { id: 'band2', label: 'Senior Leadership', range: 'Grades 6–12' },
  ...
  it('returns empty warnings when all factors are in range', () => {
    const result = scoreRole(
      companyCeiling,
      'band1',
      {
    ...
    // No warnings when all factors are in range
    expect(result.factorLevelWarnings).toBeUndefined();
  });

  it('returns warnings when factors are out of range', () => {
    const result = scoreRole(
      companyCeiling,
      'band1',
      {
        jobFunctionalKnowledge: 0,
        businessExpertise: 0,
        leadership: 0,
        problemSolving: 0,
        natureOfImpact: 0,
    ...
    expect(result.factorLevelWarnings).toBeDefined();
    expect(result.factorLevelWarnings!.length).toBeGreaterThan(0);
  });

  it('returns undefined warnings when factorLevels not provided', () => {
    const result = scoreRole(
      companyCeiling,
      'band1',
      {
